    ...
    2. JVM垃圾回收(GC)模型
        <1> 垃圾判断算法（判断内存中什么是垃圾什么不是垃圾）
            ...
            1) 引用计数算法(Reference Counting)
                * 概念理解：给对象添加一个引用计数器，当有一个地方引用它，计数器加1，当引用失效，计数器减1，任何时刻，
                           计数器为0的对象就是不可能再被使用的
...
<img src="../img/img117.png" width=800px>
        ...
        <2> 方法区
            ...
            * 在堆中，尤其在新生代，常规引用进行一次GC一般可以回收70%~95%的空间，而方法区GC效率远远小于此
...
<img src="../img/img128.png" width=800px>
        ...
        <3> 标记-整理算法(Mark-Compact)
            * 标记清除与标记整理一样，但后续步骤不是进行直接清理，而是令所有存活的对象向一端移动，然后直接清理掉这端边
              界以外的内存
